...
def _prepare_for_experiment(args):
    ...
    print("general_32_-----",args.split_dir)
...
def _save_splits(split_datasets, column_keys, filename, boolean_style=False):
    #metadata 就是处理过的 csv文件 splits最后包含两个列表 每个列表 存储的是对应的 slide_id
    splits = [split_datasets[i].metadata['slide_id'] for i in range(len(split_datasets))]
    if not boolean_style:
        #将 splits中的两个 文件框 拼接在一起  并设置 train val 的两个列名
...
def _collate_survpath(batch):
    ...
    gene_names_list = []
    for item in batch:
        gene_names_list.extend(item[7])

    return [img, omic_data_list, label, event_time, c, clinical_data_list, mask, gene_names_list]
